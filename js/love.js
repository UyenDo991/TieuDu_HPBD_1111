...
(function(window){
    ...
    function showNote() {
        setTimeout(() => {
            if (!note) {
                console.error("Note chưa sẵn sàng!");
                return;
            }
    ...
    Heart = function() {
       ...
       // 🌸 Mô phỏng bông hoa anh đào với 5 cánh mềm mại
        const points = [];
        const petals = 5;          // 5 cánh
        const step = (Math.PI * 2) / petals;
        const R = 15;              // bán kính cánh
        const petalDetail = 80;    // độ mịn cánh hoa

        for (let i = 0; i < petals; i++) {
            const baseAngle = i * step;
            for (let t = 0; t <= 1; t += 1 / petalDetail) {
                // góc lệch theo t để tạo đường cong chữ V nhẹ
                const angle = baseAngle + (t - 0.5) * 1.1;

                // độ phình tự nhiên của cánh (sin wave)
                const petalCurve = Math.sin(Math.PI * t);

                // bán kính có chỗ phình và lõm (giống cánh thật)
                const r =
                    R *
                    (0.5 + 0.5 * petalCurve) *     // phình ở giữa
                    (1 - 0.15 * Math.pow(t - 0.5, 2)) *  // lõm ở gốc
                    (1 + 0.1 * Math.sin(5 * t));         // gợn nhẹ tự nhiên
    ...
    Seed.prototype = {
        ...
        drawSakura: function() {
            var ctx = this.tree.ctx, heart = this.heart;
            var point = heart.point, color = 'rgba(255,182,193,0.9)', 
                scale = heart.scale;
            ctx.save();
            ctx.fillStyle = color;
            ctx.translate(point.x, point.y);
            ...
            for (let i = 0; i < 5; i++) {
                ...
                ctx.moveTo(0, 0);
                ctx.bezierCurveTo(10, -10, 30, -10, 30, 0);
                ctx.bezierCurveTo(30, 10, 10, 10, 0, 0);
                ctx.fillStyle = color || 'rgba(255,182,193,0.9)';
                ctx.fill();
            }
        ...
        hover: function(x, y, radius = 1000) {
            const ctx = this.tree.ctx;
            const imageData = ctx.getImageData(x - radius, y - radius, radius * 2, radius * 2);
            const pixels = imageData.data;

            // Duyệt qua toàn bộ vùng kiểm tra
            for (let i = 3; i < pixels.length; i += 4) {
                if (pixels[i] > 100) {// alpha > 100 tức là có màu vẽ
                    return true;
                }
            }
            return false;
    ...
    Tree.prototype = {
        ...
        move: function(k, x, y) {
            ...
            if (rec.speed < 2) {
                rec.speed = 2;
                //cây di chuyển xong thì hiện khung chúc
                showNote();
            }
            return i < x || j < y;
        },
    ...
    Bloom = function(tree, point, figure, color, alpha, angle, scale, place, speed) {
        this.tree = tree;
        this.point = point;
        this.color = 'rgba(255,182,193,0.9)';//color || 'rgb(255,' + random(0, 255) + ',' + random(0, 255) + ')';
    ...
    Bloom.prototype = {
        ...
        draw: function() {
            ...
            // const pistilCount = 8 + Math.floor(Math.random() * 4);
